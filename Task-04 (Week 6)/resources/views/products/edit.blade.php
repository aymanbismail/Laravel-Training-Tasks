...
<body>
  <div class="container">
    ...
    <form action="{{ route('products.update', $product) }}" method="POST">
      ...
      <!-- Suppliers Section -->
      <h2>Suppliers</h2>
      <p style="color: #666; margin-bottom: 15px; font-size: 14px;">Select at least one supplier and fill in the cost
        and lead time.</p>

      <div class="suppliers-section">
        @foreach($suppliers as $supplier)
        @php
        // Check if supplier is attached to this product
        $existingSupplier = $product->suppliers->find($supplier->id);
        $oldSupplier = old("suppliers.{$supplier->id}", []);

        // Determine if selected (old input takes priority)
        $isSelected = isset($oldSupplier['selected']) ? !empty($oldSupplier['selected']) : ($existingSupplier !== null);

        // Get cost and lead time values
